...
class windowStatus {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.msgs.forEach((val) => {
      bufferOffset = stereoLandmarks.serialize(val, buffer, bufferOffset);
    });
    ...
    obj.tracks.forEach((val) => {
      bufferOffset = interFrameTracks.serialize(val, buffer, bufferOffset);
    });
    // Serialize message field [motion]
    // Serialize the length for message field [motion]
    bufferOffset = _serializer.uint32(obj.motion.length, buffer, bufferOffset);
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.tracks[i] = interFrameTracks.deserialize(buffer, bufferOffset)
    }
    // Deserialize message field [motion]
    // Deserialize array length for message field [motion]
    len = _deserializer.uint32(buffer, bufferOffset);
    data.motion = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.motion[i] = sensor_msgs.msg.Image.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.msgs.forEach((val) => {
      length += stereoLandmarks.getMessageSize(val);
    });
    object.tracks.forEach((val) => {
      length += interFrameTracks.getMessageSize(val);
    });
    object.motion.forEach((val) => {
  ...
  static Resolve(msg) {
    ...
    if (msg.tracks !== undefined) {
      ...
      for (let i = 0; i < resolved.tracks.length; ++i) {
        resolved.tracks[i] = interFrameTracks.Resolve(msg.tracks[i]);
      }
    }
    else {
      resolved.tracks = []
    ...
    if (msg.motion !== undefined) {
      resolved.motion = new Array(msg.motion.length);
      for (let i = 0; i < resolved.motion.length; ++i) {
        resolved.motion[i] = sensor_msgs.msg.Image.Resolve(msg.motion[i]);
      }
    }
    else {
      resolved.motion = []
